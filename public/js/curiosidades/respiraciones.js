 ...
 * Textos via data-* de div#respiraciones-i18n (i18n via HTML).
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const i18n = document.getElementById('respiraciones-i18n');
  if (!i18n) return;
  ...
  btn?.addEventListener('click', () => {
    ...
    const shareText = d.shareText.replace('{n}', enMiles(total)).replace('{edad}', edad);
    ...
    resultado.innerHTML = `
      <div class="resumen-calculo curiosidad-resumen">
        ...
        <ul>
          ...
          <li><span>${d.labelPerDay}</span><span>${fmt(porDia)}</span></li>
          <li><span>${d.labelPerYear}</span><span>${fmt(porAnio)}</span></li>
